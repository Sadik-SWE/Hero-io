      ...
      <div>
        ...
        {filteredData.length === 0 ? (
          <div className="text-center mt-10 mb-10">
            ...
            <button
              onClick={handleReset}
              className="px-4 py-2 bg-linear-to-r from-purple-800 to-purple-500 text-white border-none rounded-md transition font-semibold"
            >
              Show All
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center mt-3 mb-5">
            {filteredData.map(({ id, image, title, downloads, ratingAvg }) => (
              <Link key={id} to={`/appDetails/${id}`}>
                <div className="border border-gray-200 rounded-md shadow-md p-4 w-[230px] h-[310px] transition-transform duration-300 hover:-translate-y-2">
                  <img
