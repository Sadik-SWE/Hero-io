...
const AppDetails = () => {
  ...
  // Corrected useEffect to safely check localStorage
  useEffect(() => {
    try {
      const storedApps = JSON.parse(localStorage.getItem('installedApps')) || [];
      const isInstalled = Array.isArray(storedApps) && storedApps.some((app) => app.id === appId);
      setInstalled(isInstalled);
    } catch (error) {
      console.error('Failed to read installedApps from localStorage:', error);
      setInstalled(false);
    }
  }, [appId]);
    ...
    <Suspense fallback={"Loading..."}>
      {!singleApp ? (
        <div className="max-w-5xl mx-auto text-center mt-10 mb-10">
          ...
          <Link to="/">
            <button className="btn mb-8 mt-5 px-10 bg-linear-to-r from-purple-800 to-purple-500 text-white border-none rounded-md">
              Go Back!
            </button>
          </Link>
        </div>
            ...
            <div>
              ...
              <button
                onClick={() => {
                  try {
                    const storedApps = JSON.parse(localStorage.getItem('installedApps')) || [];
                    const isAlreadyInstalled = Array.isArray(storedApps) && storedApps.some((app) => app.id === appId);
                    ...
                    toast.error('Something went wrong while installing the app.');
                  }
                }}
